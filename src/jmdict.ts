 ...
 * @returns {number}
 */
export function priCalc(argPriArray: string[]): number {
  ...
  const first = nfCheck[0];
  if (first !== undefined) {
    const nfNum = Number(first.slice(2, 4)); // Get the number from "nfxx"
...
export async function loadJMDict(
  ...
  let captures = entityRegex.exec(data);

  while (captures !== null) {
    const key = captures[1];
    const value = captures[2];

    entities[key?.toString() ?? ""] = value ?? "";

    captures = entityRegex.exec(data);
  }

  // remove entities from data
  if (shortEntities) {
    data = data.replace(/<!ENTITY (.*?) "(.*?)">/g, '<!ENTITY $1 "$1">');
  }

  const jmdictObj = parser.toJson(data, {
    object: true,
    arrayNotation: true,
  }) as unknown as JMdict;
  ...
  dict.forEach((jmdictEntry) => {
    if (jmdictEntry.k_ele) {
      jmdictEntry.k_ele.forEach((kEle) => {
        ...
        item.push(jmdictEntry);
      });
    }
  });

  return { entries, byKanji };
};
